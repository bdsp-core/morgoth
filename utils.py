...
class MarginLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        """
        true_logits = logits[torch.arange(len(targets)), targets].unsqueeze(1)

        margin_loss = torch.clamp(self.margin - (true_logits - logits), min=0)
        margin_loss[torch.arange(len(targets)), targets] = 0
...
class MorgothSmoothedValue(object):
    ...
    def synchronize_between_processes(self):
        ...
        """
        if not morgoth_is_dist_avail_and_initialized():
            return
...
class MorgothMetricLogger(object):
    def __init__(self, delimiter="\t"):
        self.meters = defaultdict(MorgothSmoothedValue)
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        iter_time = MorgothSmoothedValue(fmt='{avg:.4f}')
        data_time = MorgothSmoothedValue(fmt='{avg:.4f}')
...
class MorgothTensorboardLogger(object):
...
def _morgoth_load_checkpoint_for_ema(model_ema, checkpoint):
    """
...
def morgoth_setup_for_distributed(is_master):
    """
...
def morgoth_is_dist_avail_and_initialized():
...
def morgoth_get_world_size():
    if not morgoth_is_dist_avail_and_initialized():
        return 1
    return dist.get_world_size()


def morgoth_get_rank():
    if not morgoth_is_dist_avail_and_initialized():
        return 0
    return dist.get_rank()


def morgoth_is_main_process():
    return morgoth_get_rank() == 0


def morgoth_save_on_master(*args, **kwargs):
    if morgoth_is_main_process():
        torch.save(*args, **kwargs)


def morgoth_all_reduce(tensor, op=dist.ReduceOp.SUM, async_op=False):
    world_size = morgoth_get_world_size()
...
def morgoth_all_gather_batch(tensors):
    """
...
class MorgothGatherLayer(torch.autograd.Function):
    """
...
def morgoth_all_gather_batch_with_grad(tensors):
    """
    Performs all_gather operation on the provided tensors.
    ...
    for tensor in tensors:
        tensor_all = MorgothGatherLayer.apply(tensor)
        tensor_list.append(tensor_all)
...
def _morgoth_get_local_rank_env():
...
def _morgoth_get_world_size_env():
...
def morgoth_init_distributed_mode(args):
    if args.dist_on_itp:
        args.rank = _morgoth_get_rank_env()
        args.world_size = _morgoth_get_world_size_env()
        args.gpu = _morgoth_get_local_rank_env()
    ...
    morgoth_setup_for_distributed(args.rank == 0)


def morgoth_load_state_dict(model, state_dict, prefix='', ignore_missing="relative_position_index"):
    missing_keys = []
    unexpected_keys = []
    error_msgs = []
...
def morgoth_get_grad_norm(parameters, norm_type=2):
...
class MorgothNativeScalerWithGradNormCount:
    ...
    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True,
        ...
        if update_grad:
            if clip_grad is not None:
                ...
                self._scaler.unscale_(optimizer)
                norm = morgoth_get_grad_norm_(parameters, layer_names=layer_names)
...
def morgoth_get_grad_norm_(parameters, norm_type: float = 2.0, layer_names=None) -> torch.Tensor:
    """Gradient norm across a set of parameters.

    Rewritten from the original two-step (per-parameter norm, then norm of
    that stack) implementation to compute the common case directly from a
    single flattened concatenation of every gradient. Mathematically
    identical for any norm_type -- the p-norm of a concatenated vector
    equals ((sum of each part's p-norm raised to the p) ** (1/p)), which is
    exactly what "norm of per-parameter norms" computes -- just without
    building the intermediate per-parameter list first. The layer_names
    diagnostic path still needs individual per-parameter norms for its
    top-5 report, so that branch keeps computing them."""
    if isinstance(parameters, torch.Tensor):
        parameters = [parameters]
    ...
    if layer_names is not None:
        per_param_norms = torch.stack([torch.norm(p.grad.detach(), norm_type).to(device) for p in parameters])
        total_norm = torch.norm(per_param_norms, norm_type)
        if torch.isnan(total_norm) or torch.isinf(total_norm) or total_norm > 1.0:
            value_top, name_top = torch.topk(per_param_norms, k=5)
            print(f"Top norm value: {value_top}")
            print(f"Top norm name: {[layer_names[i][7:] for i in name_top.tolist()]}")
        return total_norm

    flat_grads = torch.cat([p.grad.detach().reshape(-1).to(device) for p in parameters])
    return torch.norm(flat_grads, norm_type)


def morgoth_cosine_scheduler(base_value, final_value, epochs, niter_per_ep, warmup_epochs=0,
                     start_warmup_value=0, warmup_steps=-1):
    """Linear warmup followed by a cosine decay from base_value to
    final_value. Rewritten to build the decay portion as one vectorized
    numpy expression (np.cos over an array of progress fractions) instead
    of a Python-level list comprehension calling math.cos once per step --
    same formula, same per-step values, just computed as an array op
    instead of an explicit per-element loop."""
    warmup_iters = warmup_steps if warmup_steps > 0 else warmup_epochs * niter_per_ep
    print("Set warmup steps = %d" % warmup_iters)
    warmup_schedule = np.linspace(start_warmup_value, base_value, warmup_iters) if warmup_epochs > 0 else np.array([])

    decay_steps = epochs * niter_per_ep - warmup_iters
    progress = np.arange(decay_steps) / decay_steps
    decay_schedule = final_value + 0.5 * (base_value - final_value) * (1 + np.cos(np.pi * progress))
...
def morgoth_save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None,
               save_ckpt_freq=1):
    ...
    if not getattr(args, 'enable_deepspeed', False):
        ...
        for checkpoint_path in checkpoint_paths:
            ...
            morgoth_save_on_master(to_save, checkpoint_path)
    else:
...
def morgoth_auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None):
    output_dir = Path(args.output_dir)
    ...
    if not getattr(args, 'enable_deepspeed', False):
        ...
        if args.resume:
            ...
            if 'optimizer' in checkpoint and 'epoch' in checkpoint:
                ...
                if hasattr(args, 'model_ema') and args.model_ema:
                    _morgoth_load_checkpoint_for_ema(model_ema, checkpoint['model_ema'])
        ...
        if args.auto_resume:
            ...
            if latest_ckpt >= 0:
                ...
                if model_ema is not None:
                    if args.model_ema:
                        _morgoth_load_checkpoint_for_ema(model_ema, client_states['model_ema'])
...
def morgoth_create_ds_config(args):
    ...
    with open(args.deepspeed_config, mode="w") as writer:
        ds_config = {
            "train_batch_size": args.batch_size * args.update_freq * morgoth_get_world_size(),
...
def morgoth_build_pretraining_dataset(datasets: list, time_window: list, stride_size=200, start_percentage=0, end_percentage=1):
    shock_dataset_list = []
    ch_names_list = []
...
def morgoth_get_input_chans(ch_names):
    input_chans = [0]
...
def morgoth_get_metrics(output, target, metrics, is_binary, threshold=0.5):
    if is_binary:
...
def spikes_results_from_1s_to_10s(predictions,data_fs, original_result_step,original_step_in_point,new_result_step_second):
    ...
    for i in range(0,len(predictions)-1,new_result_step//original_result_step):
        ...
        if len(filtered_values) > data_fs/4/original_result_step:
...
def continuous_binary_probabilities(predictions, data_fs, result_step,smooth_method = 'ema'):
    ...
    window_size = int(data_fs/5/result_step)
    step = window_size
    ...
    if smooth_method == 'ema':
        ...
        for start in range(0, len(predictions)-window_size, step):
            ...
            if np.sum(window < 0.5) > window_size:
...
def prepare_classification_dataset(root,original_format='pkl', Bipolar=False, addBipolar=False, only_evaluate=False, sub_dir=False, sample_length=False,):
    if only_evaluate:
        ...
        if 'TUEV' in result_file:
            evaluation_dataset=MorgothTUEVLoader(sub_dir, evaluation_files)
...
class MorgothTUEVLoader(torch.utils.data.Dataset):
    ...
    def __init__(self,type,
        ...
        if file_extension == '.mat':
            ...
            for old_name, new_name in new_channel_names.items():
                if counter[new_name] == 1:
                    final_channel_names[old_name] = new_name
                else:
                    final_channel_names[old_name] = old_name
        ...
        original_window_size=window_size * self.self_fs
...
class bipolar_montage():
    def __init__(self):
        ...
        self.bipolar_channels = ['FP1-F7', 'F7-T3', 'T3-T5', 'T5-O1', 'FP2-F8', 'F8-T4', 'T4-T6', 'T6-O2', 'FP1-F3', 'F3-C3', 'C3-P3', 'P3-O1', 'FP2-F4', 'F4-C4', 'C4-P4', 'P4-O2', 'FZ-CZ', 'CZ-PZ']
...
def bipolar(data_monopolar):
    ...
    data_bipolar[0, :] = data_monopolar[0, :] - data_monopolar[4, :]
    data_bipolar[1, :] = data_monopolar[4, :] - data_monopolar[5, :]
    data_bipolar[2, :] = data_monopolar[5, :] - data_monopolar[6, :]
    data_bipolar[3, :] = data_monopolar[6, :] - data_monopolar[7, :]

    data_bipolar[4, :] = data_monopolar[11, :] - data_monopolar[15, :]
    data_bipolar[5, :] = data_monopolar[15, :] - data_monopolar[16, :]
    data_bipolar[6, :] = data_monopolar[16, :] - data_monopolar[17, :]
    data_bipolar[7, :] = data_monopolar[17, :] - data_monopolar[18, :]

    data_bipolar[8, :] = data_monopolar[0, :] - data_monopolar[1, :]
    data_bipolar[9, :] = data_monopolar[1, :] - data_monopolar[2, :]
    data_bipolar[10, :] = data_monopolar[2, :] - data_monopolar[3, :]
    data_bipolar[11, :] = data_monopolar[3, :] - data_monopolar[7, :]

    data_bipolar[12, :] = data_monopolar[11, :] - data_monopolar[12, :]
    data_bipolar[13, :] = data_monopolar[12, :] - data_monopolar[13, :]
    data_bipolar[14, :] = data_monopolar[13, :] - data_monopolar[14, :]
    data_bipolar[15, :] = data_monopolar[14, :] - data_monopolar[18, :]

    data_bipolar[16, :] = data_monopolar[8, :] - data_monopolar[9, :]
    data_bipolar[17, :] = data_monopolar[9, :] - data_monopolar[10, :]
...
class SAM(torch.optim.Optimizer):
    ...
    NEW, opt-in addition. Wraps a base optimizer (e.g. AdamW) with a
    two-step update: `first_step` perturbs parameters toward the point of
    locally-worst loss within an rho-radius neighborhood, then `second_step`
    computes the real gradient at that perturbed point, restores the
    original weights, and hands the step to the base optimizer. This is a
    genuinely different training dynamic from plain AdamW (finds flatter
    minima, at the cost of one extra forward/backward pass per step).
    Nothing in create_optimizer() or the default training loop changes --
    SAM is only used when a caller explicitly opts in (see --sam_rho in
    ...
    def step(self, closure=None):
        raise RuntimeError(
            "SAM requires two explicit steps (first_step then second_step around a second "
            "forward/backward at the perturbed weights) -- use those directly instead of step().")
